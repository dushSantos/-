...
//        InputStream stream = Minecraft.getMinecraft().getResourceManager().getResource(location).getInputStream();
        InputStream stream = CoreAPI.getResourceInputStream(location.getResourceDomain() + "/" + location.getResourcePath());
    ...
    void cacheGlyphs(Font font, char text[], int start, int limit, int layoutFlags)
    {
        ...
        for(int index = 0; index < numGlyphs; index++)
        {
            ...
            if(!vectorRendered)
            {
                ...
                vectorBounds.width += 1; vectorBounds.height += 1;//Krogenit fix incorrect bounds
            ...
            rect.x -= 1; rect.y -= 1; rect.height += 2; rect.width += 2;//Krogenit fix incorrect rectangle size and position
            ...
            entry.u1 = ((float) rect.x) / (float) TEXTURE_WIDTH;
            entry.v1 = ((float) rect.y) / (float) TEXTURE_HEIGHT;
            entry.u2 = ((float) (rect.x + rect.width)) / (float) TEXTURE_WIDTH;
            entry.v2 = ((float) (rect.y + rect.height)) / (float) TEXTURE_HEIGHT;
...
//            stringGraphics.setRenderingHints(desktopHints);
//        }
    }
    ...
    private void allocateGlyphCacheTexture()
    {
         ...
         */
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, textureName);
        GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL_RGBA, TEXTURE_WIDTH, TEXTURE_HEIGHT, 0,
                GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, imageBuffer);
