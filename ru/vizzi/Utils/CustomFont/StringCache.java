...
public class StringCache
{
    ...
    public void updateHeight(){
        ...
        for(Glyph g : cacheString("AaBbCcDdEeHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZzАаБбВвГгДдЕеЁёЖжЗзИиЙйЫы1234567890").glyphs){
        ...
        fontHeight = ((maxY + height) / 2.0F - minY / 2.0F);
    }
...
//        if(shadowFlag) GL11.glColor4f(0f,0f,0f, (color >> 24 & 0xff) / 255f);
//        else GL11.glColor4f((color >> 16 & 0xff) / 255f, (color >> 8 & 0xff) / 255f, (color & 0xff) / 255f, 1);
        /*
        ...
        if(shadowFlag) tessellator.setColorRGBA(0, 0, 0, color >> 24 & 0xff);
        else tessellator.setColorRGBA(color >> 16 & 0xff, color >> 8 & 0xff, color & 0xff, color >> 24 & 0xff);
        ...
        for(int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++)
        {
            ...
            {
                if(!shadowFlag) color = applyColorCode(entry.colors[colorIndex].colorCode, initialColor, shadowFlag);
        ...
        return entry.advance / 2f + startX;
    }
    ...
    private int sizeString(String str, float width, boolean breakAtSpaces)
    {
        ...
        {
            index = wsIndex + 1;//Krogenit removing first space
        }
